...
import { FiX, FiLoader, FiSearch, FiChevronDown } from 'react-icons/fi';

const StockInModal = ({ isOpen, onClose, onSave, items: allItems, loading, record, readOnly = false, categories = [] }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (record) {
        ...
        if (item) {
          setItemSearchQuery(`${item.name} (${item.item_code})`);
        }
      } else {
        setFormData({
          itemId: '',
